...
void AlgoTrader::Logger::logTradeImpl(const AlgoTrader::LogSignal &signal, std::string_view symbol,
				      double positionSize, std::string_view strategyName)
{
	logInternal(std::format("{} {} {} {} ${:.4f}\n", getTime(), strategyName,
				convertSignalToString(signal), symbol, positionSize));
}
...
std::string AlgoTrader::Logger::convertSignalToString(const AlgoTrader::LogSignal &signal)
{
	switch (signal) {
	case AlgoTrader::LogSignal::BUY:
		return "Buy";

	case AlgoTrader::LogSignal::SELL:
		return "Sell";

	case AlgoTrader::LogSignal::HOLD:
		return "Hold";

	case AlgoTrader::LogSignal::UNDEFINED:
	default:
		return "Undefined";
	}
}
